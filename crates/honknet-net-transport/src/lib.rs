...
pub trait NetworkTransport: Send + Sync {
    ...
    async fn send(&self, peer: PeerId, channel: Channel, kind: u16, payload: &[u8]) -> Result<(), TransportError>;
...
impl UdpTransport {
    pub async fn bind(addr: SocketAddr) -> Result<Arc<Self>, TransportError> {
        let s = Arc::new(Self {
            socket: Arc::new(UdpSocket::bind(addr).await?), peers: Default::default(), by_addr: Default::default(),
            events: Default::default(), next_peer: Arc::new(Mutex::new(1)), resend: Duration::from_millis(120)
        });
        let x = s.clone();
        tokio::spawn(async move {
            x.receive_loop().await
        });
        let x = s.clone();
        tokio::spawn(async move {
            x.resend_loop().await
        ...
        loop {
            ...
            let peer = if let Some(p) = self.by_addr.lock().get(&addr).copied() {
                ...
                *id += 1;
                ...
                self.peers.lock().insert(p, Peer {
                    addr, next: 1, recv_latest: 0, recv_bits: 0, sent: HashMap::new(), assembler: Default::default(),
                    last_seen: Instant::now()
                });
                self.events.lock().push_back(TransportEvent::Connected(p, addr));
                p
            };
            ...
            if let Some(full) = state.assembler.push(h, payload) {
                self.events.lock().push_back(TransportEvent::Data(peer, h.channel, h.kind, full));
            }
        }
    }
    async fn resend_loop(self: Arc<Self>) {
        ...
        loop {
            ...
            let packets: Vec<_> = {
                let mut peers = self.peers.lock();
                peers.values_mut().flat_map(|p| p.sent.values_mut().filter(|s| now.duration_since(s.at) >= self.resend).flat_map(|s| {
                    s.at = now;
                    s.packets.iter().cloned().map(|x|(p.addr, x)).collect::<Vec<_>>()
                })).collect()
            };
    ...
    pub async fn connect(&self, addr: SocketAddr) -> PeerId {
        ...
        *id += 1;
        self.by_addr.lock().insert(addr, p);
        self.peers.lock().insert(p, Peer {
            addr, next: 1, recv_latest: 0, recv_bits: 0, sent: HashMap::new(), assembler: Default::default(), last_seen: Instant::now()
        });
        p
    }
}
...
impl NetworkTransport for UdpTransport {
    ...
    async fn send(&self, peer: PeerId, channel: Channel, kind: u16, payload: &[u8]) -> Result<(), TransportError> {
        ...
        let reliable = matches!(channel, Channel::ReliableOrdered | Channel::ReliableUnordered | Channel::Control | Channel::BulkTransfer);
        let packets = honknet_net_core::fragment(kind, channel, if reliable {
            PacketFlags::RELIABLE
        } else {
            PacketFlags::empty()
        }, seq, ack, bits, 0, payload, seq as u16);
        for x in &packets {
            self.socket.send_to(x, addr).await?;
        }
        if reliable {
            self.peers.lock().get_mut(&peer).ok_or(TransportError::PeerMissing)?.sent.insert(seq, Sent {
                at: Instant::now(), packets
            });
        }
        Ok(())
    }
    async fn disconnect(&self, peer: PeerId, reason: &str) -> Result<(), TransportError> {
        let p = self.peers.lock().remove(&peer).ok_or(TransportError::PeerMissing)?;
        self.by_addr.lock().remove(&p.addr);
        self.events.lock().push_back(TransportEvent::Disconnected(peer, reason.into()));
        Ok(())
    }
}
...
impl LoopbackTransport {
    pub fn pair() ->(Arc<Self>, Arc<Self>) {
        ...
        (Arc::new(Self {
            tx: b_tx, rx: tokio::sync::Mutex::new(a_rx), peer: 2
        }), Arc::new(Self {
            tx: a_tx, rx: tokio::sync::Mutex::new(b_rx), peer: 1
        }))
    }
}
...
impl NetworkTransport for LoopbackTransport {
    ...
    async fn send(&self, _: PeerId, ch: Channel, kind: u16, payload: &[u8]) -> Result<(), TransportError> {
        self.tx.send(TransportEvent::Data(self.peer, ch, kind, payload.to_vec())).await.map_err(|_| TransportError::Closed)
    }
    async fn disconnect(&self, _: PeerId, reason: &str) -> Result<(), TransportError> {
        self.tx.send(TransportEvent::Disconnected(self.peer, reason.into())).await.map_err(|_| TransportError::Closed)
    }
}
...
pub async fn binary_websocket_connect(url: &str) -> Result<tokio_tungstenite::WebSocketStream<tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>>,
TransportError> {
    let(ws, _) = tokio_tungstenite::connect_async(url).await.map_err(|e| std::io::Error::other(e.to_string()))?;
    Ok(ws)
}
