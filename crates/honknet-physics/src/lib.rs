...
impl Shape {
    pub fn aabb(&self, p: Vec2) -> Aabb {
        match self {
            ...
            => Aabb::from_center_half(p, Vec2::new(*radius, *radius)),
            Shape::Capsule {
                half_height,
                radius
            }
            ...
            => children.iter().map(|(o, s)| s.aabb(p + *o)).reduce(|a, b| a.union(b)).unwrap_or(Aabb::from_center_half(p,
            Vec2::ZERO))
        }
    }
}
...
impl Body {
    ...
    pub fn aabb(&self) -> Aabb {
        self.fixtures.iter().map(|f| f.shape.aabb(self.position)).reduce(|a, b| a.union(b)).unwrap_or(Aabb::from_center_half(self.position,
        Vec2::ZERO))
    }
}
...
impl PhysicsWorld {
    ...
    pub fn raycast(&self, origin: Vec2, dir: Vec2, max: f32, mask: u32) -> Option<(Entity, f32, Vec2)> {
    ...
    pub fn shape_cast(&self, shape: &Shape, start: Vec2, delta: Vec2, mask: u32) -> Option<(Entity, f32)> {
        let steps = 32;
        for i in 0..=steps {
            ...
            for e in self.index.query_aabb(a) {
                let b = &self.bodies[&e];
                if b.fixtures.iter().any(|f| f.layer & mask != 0 && a.intersects(f.shape.aabb(b.position))) {
                    return Some((e, t))
                }
            }
        }
    ...
    for fa in &a.fixtures {
        for fb in &b.fixtures {
            ...
            if let Some((n, p, pt)) = shape_contact(&fa.shape, a.position, &fb.shape, b.position) {
                return Some(Contact {
                    a: a.entity, b: b.entity, normal: n, penetration: p, point: pt, sensor: fa.sensor || fb.sensor
                })
            }
        }
    }
...
fn ray_aabb(o: Vec2, d: Vec2, a: Aabb) -> Option<f32> {
    let inv = Vec2::new(if d.x.abs() > 1e-6 {
        1. / d.x
    } else {
        f32::INFINITY
    }, if d.y.abs() > 1e-6 {
...
mod tests {
    ...
    fn circles_resolve() {
        ...
        p.insert(Body::dynamic(Entity::new(0, 0), Vec2::ZERO, 1., f(Shape::Circle {
            radius: 1.
        })));
        p.insert(Body::dynamic(Entity::new(1, 0), Vec2::new(1.5, 0.), 1., f(Shape::Circle {
            radius: 1.
        })));
        p.step(0.016);
