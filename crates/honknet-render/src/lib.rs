        ...
        self.layers
            ...
            .filter(|s| {
                Aabb::from_center_half(
                    Vec2::new(s.position[0], s.position[1]),
                    Vec2::new(s.size[0], s.size[1]) * 0.5,
                )
                .intersects(view)
            })
            .copied()
...
impl Atlas {
    ...
    pub fn insert(&mut self, key: &str, w: u32, h: u32) -> Option<[f32; 4]> {
        ...
        let r = [
            self.next[0] as f32 / self.size[0] as f32,
            self.next[1] as f32 / self.size[1] as f32,
            w as f32 / self.size[0] as f32,
...
impl WgpuRenderer {
    ...
    pub fn render(&mut self, sprites: &[SpriteInstance], clear: [f64; 4]) {
        ...
        self.queue
            .write_buffer(&self.instance, 0, bytemuck::cast_slice(sprites));
