...
impl Localization {
    ...
    pub fn format(&self, locale: &str, key: &str, args: &HashMap<String, String>) -> Result<String, LocalizationError> {
        let maps = self.locales.read();
        let fallback = self.fallback.read();
        let mut v = maps.get(locale).and_then(|m| m.get(key)).or_else(||maps.get(& *fallback).and_then(|m| m.get(key))).cloned().ok_or_else(||LocalizationError::Missing(key.into()))?;
        for (k, x) in args {
