...
type MapTool = 'brush' | 'erase' | 'fill' | 'rectangle' | 'entity' | 'select' | 'pan';

const TILE_DEFINITIONS = [
  { id: 'floor', label: 'Floor', color: '#283843' },
  { id: 'wall', label: 'Wall', color: '#7a8992' },
  { id: 'road', label: 'Road', color: '#20252b' },
  { id: 'sidewalk', label: 'Sidewalk', color: '#596269' },
  { id: 'grass', label: 'Grass', color: '#294d35' },
  { id: 'water', label: 'Water', color: '#164d70' },
  { id: 'sand', label: 'Sand', color: '#826d3d' },
  { id: 'void', label: 'Void', color: '#07090c' },
] as const;

const TILE_COLORS = new Map<string, string>(TILE_DEFINITIONS.map((tile) => [tile.id, tile.color]));
...
export class MapEditor extends ModelEditor<MapModel> {
  ...
  public constructor(source: string, path: string, metadata: ProjectMetadata) {
    ...
    this.selectedPrototype = metadata.prototypes[0] ?? 'DebugPlayer';
  }

  public static create(name: string, width: number, height: number, metadata: ProjectMetadata): MapEditor {
    ...
    return new MapEditor(YAML.stringify(toMapDocument(model), { lineWidth: 0 }), `${name}.yml`, metadata);
  }
  ...
  protected renderDesigner(): void {
    ...
    toolbar.append(element('span', { className: 'toolbar-separator' }));
    ...
    const renderItems = (): void => {
      ...
      if (this.tool === 'entity') {
        ...
        for (const prototype of prototypes) {
          ...
          item.prepend(element('span', { className: 'entity-glyph', text: prototype.slice(0, 1).toUpperCase() }));
          list.append(item);
        }
        if (prototypes.length === 0) list.append(element('p', { className: 'empty-state', text: 'No prototypes found.' }));
      } else {
        for (const tile of TILE_DEFINITIONS.filter((entry) => entry.label.toLowerCase().includes(query) || entry.id.includes(query))) {
          ...
          const swatch = element('span', { className: 'tile-swatch' });
  ...
  private installCanvasEvents(canvas: HTMLCanvasElement): void {
    ...
    canvas.addEventListener('keydown', (event) => {
      ...
      const shortcuts: Record<string, MapTool> = { v: 'select', b: 'brush', e: 'erase', f: 'fill', r: 'rectangle', n: 'entity' };
  ...
  private applyTool(x: number, y: number, withHistory = true): void {
    ...
    if (this.tool === 'brush' || this.tool === 'erase') {
      ...
      if (this.model.tiles[y]?.[x] === nextTile) return;
      const paint = (model: MapModel): void => { const row = model.tiles[y]; if (row) row[x] = nextTile; };
  ...
  private draw(): void {
    ...
    if (this.tilesVisible) {
      for (let y = 0; y < this.model.height; y += 1) {
        ...
        for (let x = 0; x < this.model.width; x += 1) {
          ...
          context.fillStyle = TILE_COLORS.get(tile) ?? colorFromString(tile);
          context.fillRect(x * cell, y * cell, cell + 0.5, cell + 0.5);
          if (this.collisionVisible && (tile === 'wall' || tile.toLowerCase().includes('wall'))) {
    ...
    if (this.entitiesVisible) {
      for (const [index, entity] of this.model.entities.entries()) {
        ...
        context.strokeStyle = selected ? '#ffffff' : '#071014';
    ...
    if (this.cursorTile) {
      ...
      if (this.tool === 'rectangle' && start) {
        const minX = Math.min(start.x, this.cursorTile.x);
        const minY = Math.min(start.y, this.cursorTile.y);
        const width = Math.abs(start.x - this.cursorTile.x) + 1;
        const height = Math.abs(start.y - this.cursorTile.y) + 1;
