# Space Station 15 — полный дизайн-документ и архитектура

**Проект:** `space-station-15`  
**Организация:** `Open Station`  
...
**Space Station 15** — это browser-first open-source framework/engine для многопользовательских 2D immersive simulation sandbox/RP игр.
...
Название **Space Station 15** сохраняется как бренд и историческое имя проекта, но технически `space-station-15` должен быть универсальной платформой для создания разных 2D сетевых симуляционных игр:
...
Repository: space-station-15
Full identity: Open Station / space-station-15
```
...
Так как проект называется Space Station 15, платформа должна позволять сделать space station сборку.
...
- создать `space-station-15` monorepo;
...
```text
Space Station 15 is not a Space Station 14 fork.
Space Station 15 is not a space-station-only engine.
Space Station 15 is a universal web-first platform for deep 2D multiplayer simulation games.
```
