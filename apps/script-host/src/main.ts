...
import { CommandBuffer, type ServerGameModule } from '@honknet/server';
import type { GameEvent } from '@honknet/shared';

type EngineMessage =
  | { type: 'Initialize'; data: { engine_version: string; module_path: string } }
  | { type: 'Tick'; data: { tick: number; delta_seconds: number; events: GameEvent[] } }
  | { type: 'Shutdown' };
