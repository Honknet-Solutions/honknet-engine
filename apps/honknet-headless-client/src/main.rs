...
use honknet_net_core::{encode_message, Channel, NetworkMessage};
use honknet_net_transport::{NetworkTransport, UdpTransport};
...
async fn main() -> Result<()> {
    ...
    for seq in 0..a.ticks {
        ...
        t.send(p, Channel::UnreliableSequenced, Input::ID, &b)
            .await?;
        let _ = t.poll().await?;
